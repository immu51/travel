    ...
    <div className="font-body text-text antialiased bg-bg">
      ...
      <main className="min-h-screen">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
          <Link
            to="/same-day-tours"
            className="inline-flex items-center gap-2 text-primary/80 hover:text-primary text-sm font-medium mb-6 transition-colors"
          >
          ...
          <div className="grid lg:grid-cols-3 gap-10">
            <div className="lg:col-span-2 space-y-8">
              {/* Hero image */}
              <header className="relative h-[280px] md:h-[360px] rounded-2xl overflow-hidden shadow-soft">
                <img
                  src={image}
                  alt={alt || title}
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6 text-white">
                  <h1 className="font-heading font-bold text-2xl sm:text-3xl md:text-4xl drop-shadow-lg">
                    {title}
                  </h1>
              ...
              {highlights?.length > 0 && (
                <section>
                  <h2 className="font-heading font-semibold text-xl text-primary mb-3">Highlights</h2>
              ...
              {itinerary?.length > 0 && (
                <section>
                  <h2 className="font-heading font-semibold text-xl text-primary mb-4">Itinerary</h2>
                  <div className="space-y-4">
                    {itinerary.map((step, i) => (
                      <div
                        key={i}
                        className="flex gap-4 rounded-2xl bg-white/80 border border-primary/10 p-4 shadow-soft"
                      >
                        <div className="flex-shrink-0 w-20 text-accent font-semibold text-sm">
                          {step.time}
                        </div>
                        <div>
                          <h3 className="font-heading font-semibold text-primary mb-1">{step.title}</h3>
                          <p className="text-text/90 text-sm leading-relaxed">{step.description}</p>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <div className="lg:col-span-1">
              <div className="lg:sticky lg:top-24">
                <AnimateIn variant="fadeUpScale" className="rounded-2xl overflow-hidden bg-white/90 backdrop-blur border border-primary/10 shadow-glass p-6">
                  <h3 className="font-heading font-semibold text-xl text-primary mb-4">Book this tour</h3>
                  {duration && (
