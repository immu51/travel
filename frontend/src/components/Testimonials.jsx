/**
 * Reviews block: public reviews (like Google Maps). All visitors see all reviews.
...
export default function Testimonials() {
  ...
  useEffect(() => {
    ...
    setLoading(false)
  }, [useFirebase, useBackendApi, loadReviews])
  ...
  const handleSubmit = async (e) => {
    ...
    if (useBackendApi) {
      ...
      setMessage(null)
      try {
        const added = await addReviewApi({ name, stars: form.stars, quote })
        if (added && added.id) {
          setReviews((prev) => [{ ...added, image: added.image || null }, ...prev])
          setForm({ name: '', stars: 5, quote: '' })
          setMessage({ type: 'success', text: 'Thank you! Your review is now live and visible to everyone, just like Google reviews.' })
        } else {
          setMessage({ type: 'error', text: 'Could not save review. Please try again.' })
        }
      } catch (_) {
        setMessage({ type: 'error', text: 'Could not save review. Please try again.' })
      } finally {
    ...
    saveReviews(next)
    setForm({ name: '', stars: 5, quote: '' })
    setMessage({ type: 'success', text: 'Thank you! Your review is now visible to everyone.' })
    setTimeout(() => setMessage(null), 4000)
  }
      ...
      <div className={CONTAINER_CLASS}>
        ...
        <AnimateIn variant="fadeUpScale" delay={150} className="mb-16">
          <div className="max-w-xl mx-auto p-6 md:p-8 rounded-2xl bg-white shadow-soft border border-primary/5">
            ...
            <p className="text-sm text-text/70 mb-4">Your review will be visible to all visitors, just like on Google.</p>
